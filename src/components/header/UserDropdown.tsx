...
export default function UserDropdown() {
  ...
  useEffect(() => {
    const refresh = () => {
      ...
      setDisplayName(name || user.username || "ผู้ใช้");
      const parts: string[] = [];
      if (user.username) parts.push(user.username);
      if (user.email) parts.push(user.email);
      const allDepts = getUserDepartmentCodes();
      const active = getUserDepartmentCode();
      if (allDepts.length > 1) {
        parts.push(`แผนก ${active ?? "—"} (${allDepts.join(", ")})`);
      } else if (active) {
        parts.push(`แผนก ${active}`);
      }
  ...
  const shortLabel =
    displayName.length > 18 ? `${displayName.slice(0, 16)}…` : displayName || "ผู้ใช้";
    ...
    <div className="relative">
      ...
      >
        <div>
          ...
          <span className="mt-0.5 block text-theme-xs text-gray-500 dark:text-gray-400 break-words">
            {subtitle || "—"}
          </span>
        </div>
