...
import {
  fetchFgLotTraceReport,
  type FgLotTraceReportFilters,
} from "@/services/productFgLotService";
...
const STATUS_OPTIONS = [
  ...
  { value: "PARTIAL_USED", label: "จ่ายออกบางส่วน" },
  { value: "USED_UP", label: "จ่ายออกครบแล้ว" },
];

export default function FgLotTraceReportPage() {
  ...
  const loadReport = useCallback(async (pageOverride?: number) => {
    ...
    try {
      const filters: FgLotTraceReportFilters = {
      ...
      const result = await fetchFgLotTraceReport(filters);
  ...
  const handleExportExcel = () => {
    ...
    XLSX.utils.book_append_sheet(wb, ws, "FG Lot Trace");
    XLSX.writeFile(
      wb,
      `FgLotTrace_${new Date().toLocaleDateString("th-TH")}.xlsx`,
    );
  };
  ...
  const handleExportCSV = () => {
    ...
    link.download = `FgLotTrace_${new Date().toLocaleDateString("th-TH")}.csv`;
    link.click();
  };
  ...
  const handleExportPDF = async () => {
    ...
    for (let pi = 0; pi < productGroups.length; pi++) {
      ...
      for (let li = 0; li < pg.lots.length; li++) {
        ...
        doc.setFontSize(10);
        doc.setFont(THAI_PDF_FONT, "bold");
        doc.text(`FG Lot Trace : ${g.lotNo}`, margin, y);
        y += 5;
        doc.setFont(THAI_PDF_FONT, "normal");
        doc.setFontSize(8);
        doc.text(
          `${g.orderNo} · ${fgLotStatusLabelTh(g.lotStatus)}`,
          margin,
          y,
        );
        y += 7;
        ...
        autoTable(doc, {
          ...
          body: g.steps.map((s) => lotTraceStepRowCells(s, g.unit, false)),
    ...
    doc.save(`FgLotTrace_${new Date().toISOString().split("T")[0]}.pdf`);
  };

  return (
    <div>
      <PageBreadcrumb pageTitle="สอบกลับล็อต FG (รับเข้า-จ่ายออก)" />
      <div className="space-y-6">
        <div className="rounded-xl border border-gray-200 bg-white p-6 shadow-sm dark:border-gray-700 dark:bg-gray-800">
          <h2 className="text-xl font-semibold text-gray-900 dark:text-white">
            รายงานสอบกลับล็อตสินค้าพร้อมขาย (FG)
          </h2>
          <p className="mt-1 text-sm text-gray-600 dark:text-gray-400">
            แสดงรายการสรุปตามรหัสสินค้า — กดดูล็อต แล้วกดดู Lot Trace (รับเข้าจากผลิต · จ่ายออกเมื่อตัดขาย)
          </p>
        </div>
        ...
        <ComponentCard
          title={`FG Lot Trace (${productGroups.length} รหัสสินค้า · ${groups.length} ล็อต · ${rowCount} แถว · ทั้งหมด ${total} ล็อต)`}
        >
          {groups.length > 0 && (
