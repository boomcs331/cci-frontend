...
export default function ProductionStepsHubPage() {
  ...
  if (loading && products.length === 0) {
    return (
      <div>
        <PageBreadcrumb pageTitle="ลำดับขั้นตอนผลิต" />
      ...
      <div className="space-y-6">
        <ComponentCard title={`ลำดับขั้นตอนผลิต (${pagination?.total ?? products.length})`}>
          <p className="text-sm text-gray-600 dark:text-gray-400 mb-4">
            เลือกสินค้าเพื่อไปหน้า BOM — กำหนดลำดับขั้นตอนผลิตได้หลังมี BOM อย่างน้อย 1 รายการ
          </p>
                  ...
                  {products.map((prod) => (
                    <tr key={prod.id} className="hover:bg-gray-50 dark:hover:bg-gray-800">
                      ...
                      <td className="px-4 py-3 text-center whitespace-nowrap">
                        <Link
                          href={`/production/products/${prod.id}/bom`}
