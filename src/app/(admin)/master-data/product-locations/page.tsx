...
export default function ProductLocationsPage() {
  ...
  const [editingItem, setEditingItem] = useState<ProductLocation | null>(null);
  const [formData, setFormData] = useState({ code: "", name: "", description: "" });
  const [message, setMessage] = useState<{ type: "success" | "error"; text: string } | null>(null);

  const fetchData = async () => {
    try {
      const response = await fetch(getApiUrl(`/masters/products/locations`));
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    try {
      const url = editingItem ? getApiUrl(`/masters/products/locations/${editingItem.id}`) : getApiUrl("/masters/products/locations");
  ...
  const handleEdit = (item: ProductLocation) => {
    setEditingItem(item);
  ...
  const handleDelete = async (id: number) => {
    ...
    try {
      const response = await fetch(getApiUrl(`/masters/products/locations/${id}`), { method: "DELETE" });
      if (response.ok) {
  ...
  return (
    <div>
      <PageBreadcrumb pageTitle="สถานที่เก็บผลิตภัณฑ์" />
      <div className="space-y-4">
        {message && <Alert variant={message.type} title={message.type === "success" ? "สำเร็จ" : "ข้อผิดพลาด"} message={message.text} />}
        <ComponentCard title={`สถานที่เก็บผลิตภัณฑ์ทั้งหมด (${apiResponse?.pagination?.total || 0})`}>
          <div className="flex flex-col lg:flex-row justify-between items-start lg:items-center gap-3 mb-3">
            ...
            <button onClick={() => { setShowModal(true); setEditingItem(null); setFormData({ code: "", name: "", description: "" }); }} className="px-3 py-1.5 text-sm bg-blue-600 text-white rounded-lg hover:bg-blue-700">เพิ่มสถานที่</button>
          </div>
        ...
        <div className="fixed inset-0 bg-black/80 flex items-center justify-center z-[99999] p-4">
          <div className="bg-white dark:bg-gray-800 rounded-xl shadow-2xl w-full max-w-md border border-gray-200 dark:border-gray-700">
            <div className="px-5 py-3 border-b border-gray-200 dark:border-gray-700 flex justify-between items-center">
              <h3 className="text-lg font-semibold text-gray-900 dark:text-white">{editingItem ? "แก้ไข" : "เพิ่ม"}สถานที่เก็บผลิตภัณฑ์</h3>
