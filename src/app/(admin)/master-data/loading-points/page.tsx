...
function PageContent() {
  ...
  const fetchData = useCallback(async () => {
    try {
      const response = await apiFetch(`/masters/loading-points?page=${page}&limit=${limit}`);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    try {
      const url = editingItem ? `/masters/loading-points/${editingItem.id}` : "/masters/loading-points";
  ...
  const handleDelete = async (id: number) => {
    ...
    try {
      const response = await apiFetch(`/masters/loading-points/${id}`, { method: "DELETE" });
      if (response.ok) {
