...
function PageContent() {
  ...
  const [editingItem, setEditingItem] = useState<LoadingPoint | null>(null);
  const [formData, setFormData] = useState({ code: "", name: "" });
  const [message, setMessage] = useState<{ type: "success" | "error"; text: string } | null>(null);

  const fetchData = async () => {
    try {
      const response = await fetch(getApiUrl(`/masters/loading-points?page=${page}&limit=${limit}`));
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    try {
      const url = editingItem ? getApiUrl(`/masters/loading-points/${editingItem.id}`) : getApiUrl("/masters/loading-points");
  ...
  const handleEdit = (item: LoadingPoint) => {
    setEditingItem(item);
  ...
  const handleDelete = async (id: number) => {
    ...
    try {
      const response = await fetch(getApiUrl(`/masters/loading-points/${id}`), { method: "DELETE" });
      if (response.ok) {
  ...
  return (
    <div>
      <PageBreadcrumb pageTitle="จุดขนถ่าย" />
      <div className="space-y-6">
        {message && <Alert variant={message.type} title={message.type === "success" ? "สำเร็จ" : "ข้อผิดพลาด"} message={message.text} />}
        <ComponentCard title={`จุดขนถ่ายทั้งหมด (${apiResponse?.data?.total || 0})`}>
          <div className="flex flex-col lg:flex-row justify-between items-start lg:items-center gap-4 mb-4">
            <PaginationSelector currentLimit={limit} />
            <button onClick={() => { setShowModal(true); setEditingItem(null); setFormData({ code: "", name: "" }); }} className="px-3 py-1.5 text-sm bg-blue-600 text-white rounded-lg hover:bg-blue-700">เพิ่มจุดขนถ่าย</button>
          </div>
