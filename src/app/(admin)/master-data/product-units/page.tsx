...
export default function Page() {
  ...
  const fetchData = async () => {
    try {
      const response = await fetch(getApiUrl(`/masters/products/units`));
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    try {
      const url = editingItem ? `/masters/products/units/${editingItem.id}` : "/masters/products/units";
  ...
  const handleDelete = async (id: number) => {
    ...
    try {
      const response = await fetch(getApiUrl(`/masters/products/units/${id}`), { method: "DELETE" });
      if (response.ok) {
        ...
        <div className="fixed inset-0 bg-black/80 flex items-center justify-center z-[99999] p-4">
          <div className="bg-white dark:bg-gray-800 rounded-xl shadow-2xl w-full max-w-md border border-gray-200 dark:border-gray-700">
            <div className="px-5 py-3 border-b border-gray-200 dark:border-gray-700 flex justify-between items-center">
              <h3 className="text-lg font-semibold text-gray-900 dark:text-white">{editingItem ? "แก้ไข" : "เพิ่ม"}หน่วย</h3>
