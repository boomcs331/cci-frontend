...
export default function UnitsPage() {
  ...
  const fetchData = async () => {
    try {
      const response = await fetch(getApiUrl(`/masters/units?page=${page}&limit=${limit}`));
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    try {
      const url = editingItem ? getApiUrl(`/masters/units/${editingItem.id}`) : getApiUrl("/masters/units");
  ...
  const handleDelete = async (id: number) => {
    ...
    try {
      const response = await fetch(getApiUrl(`/masters/units/${id}`), { method: "DELETE" });
      if (response.ok) {
  ...
  return (
    <div>
      <PageBreadcrumb pageTitle="หน่วย" />
      <div className="space-y-6">
        {message && <Alert variant={message.type} title={message.type === "success" ? "สำเร็จ" : "ข้อผิดพลาด"} message={message.text} />}
        <ComponentCard title={`หน่วยทั้งหมด (${apiResponse?.data?.total || 0})`}>
          <div className="flex flex-col lg:flex-row justify-between items-start lg:items-center gap-4 mb-4">
            <PaginationSelector currentLimit={limit} />
            <button onClick={() => { setShowModal(true); setEditingItem(null); setFormData({ code: "", name: "" }); }} className="px-3 py-1.5 text-sm bg-blue-600 text-white rounded-lg hover:bg-blue-700">เพิ่มหน่วย</button>
          </div>
