...
function PageContent() {
  ...
  const fetchData = async () => {
    try {
      const response = await fetch(getApiUrl(`/masters/products/types`));
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    try {
      const url = editingItem ? getApiUrl(`/masters/products/types/${editingItem.id}`) : getApiUrl("/masters/products/types");
  ...
  const handleDelete = async (id: number) => {
    ...
    try {
      const response = await fetch(getApiUrl(`/masters/products/types/${id}`), { method: "DELETE" });
      if (response.ok) {
