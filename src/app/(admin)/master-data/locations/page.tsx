...
function PageContent() {
  ...
  const confirmDelete = async () => {
    if (!deleteId) return;
    try {
      const response = await apiFetch(`/masters/materials-locations/${deleteId}`, { method: "DELETE" });
      if (response.ok) {
  ...
  return (
    <PageContainer>
      ...
      <BaseModal
        ...
        title={`${editingItem ? "แก้ไข" : "เพิ่ม"}สถานที่เก็บ`}
        size="sm"
      >
      ...
      {deleteId !== null && (
        ...
        >
          <div className="mb-6 text-gray-700 dark:text-gray-300">
            คุณต้องการลบสถานที่เก็บรายการนี้หรือไม่?
          </div>
