...
export default function LocationsPage() {
  ...
  const fetchData = async () => {
    try {
      const response = await fetch(getApiUrl(`/masters/materials-locations`));
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    try {
      const url = editingItem ? getApiUrl(`/masters/materials-locations/${editingItem.id}`) : getApiUrl("/masters/materials-locations");
  ...
  const handleDelete = async (id: number) => {
    ...
    try {
      const response = await fetch(getApiUrl(`/masters/materials-locations/${id}`), { method: "DELETE" });
      if (response.ok) {
  ...
  return (
    <div>
      <PageBreadcrumb pageTitle="สถานที่เก็บ" />
      <div className="space-y-4">
        {message && <Alert variant={message.type} title={message.type === "success" ? "สำเร็จ" : "ข้อผิดพลาด"} message={message.text} />}
        <ComponentCard title={`สถานที่เก็บทั้งหมด (${apiResponse?.pagination?.total || 0})`}>
        ...
        <div className="fixed inset-0 bg-black/80 flex items-center justify-center z-[99999] p-4">
          <div className="bg-white dark:bg-gray-800 rounded-xl shadow-2xl w-full max-w-md border border-gray-200 dark:border-gray-700">
            <div className="px-5 py-3 border-b border-gray-200 dark:border-gray-700 flex justify-between items-center">
              <h3 className="text-lg font-semibold text-gray-900 dark:text-white">{editingItem ? "แก้ไข" : "เพิ่ม"}สถานที่เก็บ</h3>
