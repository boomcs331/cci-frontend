...
function PageContent() {
  ...
  const fetchData = useCallback(async () => {
    try {
      const response = await apiFetch(`/masters/models`);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    try {
      const url = editingItem ? `/masters/models/${editingItem.id}` : "/masters/models";
  ...
  const handleDelete = async (id: number) => {
    ...
    try {
      const response = await apiFetch(`/masters/models/${id}`, { method: "DELETE" });
      if (response.ok) {
