...
export default function ProcessLinesPage() {
  ...
  const [editingItem, setEditingItem] = useState<ProcessLine | null>(null);
  const [formData, setFormData] = useState({ code: "", name: "" });
  const [message, setMessage] = useState<{ type: "success" | "error"; text: string } | null>(null);

  const fetchData = async () => {
    try {
      const response = await fetch(getApiUrl(`/masters/process-lines?page=${page}&limit=${limit}`));
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    try {
      const url = editingItem ? getApiUrl(`/masters/process-lines/${editingItem.id}`) : getApiUrl("/masters/process-lines");
  ...
  const handleDelete = async (id: number) => {
    ...
    try {
      const response = await fetch(getApiUrl(`/masters/process-lines/${id}`), { method: "DELETE" });
      if (response.ok) {
  ...
  return (
    <div>
      <PageBreadcrumb pageTitle="สายการผลิต" />
      <div className="space-y-6">
        {message && <Alert variant={message.type} title={message.type === "success" ? "สำเร็จ" : "ข้อผิดพลาด"} message={message.text} />}
        <ComponentCard title={`สายการผลิตทั้งหมด (${apiResponse?.data?.total || 0})`}>
          <div className="flex flex-col lg:flex-row justify-between items-start lg:items-center gap-4 mb-4">
            <PaginationSelector currentLimit={limit} />
            <button onClick={() => { setShowModal(true); setEditingItem(null); setFormData({ code: "", name: "" }); }} className="px-3 py-1.5 text-sm bg-blue-600 text-white rounded-lg hover:bg-blue-700">เพิ่มสายการผลิต</button>
          </div>
          ...
          {apiResponse?.data && <PaginationFooter page={apiResponse.data.page} limit={apiResponse.data.limit} total={apiResponse.data.total} totalPages={apiResponse.data.totalPages} />}
        </ComponentCard>
      </div>
