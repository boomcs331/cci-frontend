...
function PageContent() {
  ...
  const fetchData = useCallback(async () => {
    try {
      const response = await apiFetch(`/masters/products/customers`);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    try {
      const url = editingItem ? `/masters/products/customers/${editingItem.id}` : "/masters/products/customers";
  ...
  const handleDelete = async (id: number) => {
    ...
    try {
      const response = await apiFetch(`/masters/products/customers/${id}`, { method: "DELETE" });
      if (response.ok) {
