...
export default function CustomersPage() {
  ...
  const fetchData = async () => {
    try {
      const response = await fetch(getApiUrl(`/masters/products/customers`));
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    try {
      const url = editingItem ? getApiUrl(`/masters/products/customers/${editingItem.id}`) : getApiUrl("/masters/products/customers");
  ...
  const handleDelete = async (id: number) => {
    ...
    try {
      const response = await fetch(getApiUrl(`/masters/products/customers/${id}`), { method: "DELETE" });
      if (response.ok) {
      ...
      <div className="space-y-4">
        ...
        <ComponentCard title={`ลูกค้าทั้งหมด (${apiResponse?.pagination?.total || 0})`}>
          <div className="flex flex-col lg:flex-row justify-between items-start lg:items-center gap-3 mb-3">
            ...
            <button onClick={() => { setShowModal(true); setEditingItem(null); setFormData({ code: "", name: "", description: "" }); }} className="px-3 py-1.5 text-sm bg-blue-600 text-white rounded-lg hover:bg-blue-700">เพิ่มลูกค้า</button>
          </div>
