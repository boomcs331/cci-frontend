...
export default function PlanDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const handleConfirm = async () => {
    if (!confirm('ต้องการจัดงานจ่ายออกวัตถุดิบหรือไม่? (จะตัด stock จริง)')) return;
    
    setLoading(true);
    try {
      const res = await fetch(`http://localhost:3006/production-plans/${id}/issue`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' }
      });
      ...
      alert(err.message || 'เกิดข้อผิดพลาดในการจัดงาน');
    } finally {
      setLoading(false);
    }
  };
